    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            if (!isAvailable) {
                ...
                const { data } = await axios.post(`/api/bookings/book`, { room: id, checkInDate, checkOutDate, guests, paymentMethod: "Pay At Hotel" }, { headers: { Authorization: `Bearer ${getToken()}` } },)
        ...
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <div className='flex flex-col lg:flex-row mt-6 gap-6'>
                <div className='lg:w-1/2 w-full'>
                    <img src={`${baseURL}${mainImage}`} alt="Room Image" className='w-full rounded-xl shadow-xl object-cover' />
                </div>
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((image, i) => (
                        <img onClick={() => setMainImage(image)}
                            key={i} src={`${baseURL}${image}`} alt="Room Image"
