...
const MyBookings = () => {
  ...
  const fetchUserBookings = async () => {
    try {
      const {data} = await axios.get('/api/booking/user', {headers: {Authorization: `Bearer ${getToken()}`}})
      console.log(data)
      if(data.success) {
  ...
  const handlePayment = async (bookingId) => {
    try {
      const {data} = await axios.post(`/api/bookings/stripe-payment`, {bookingId}, {headers: {Authorization: `Bearer ${getToken()}`}})
      if(data.success) {
